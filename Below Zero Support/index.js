...
function testMod(files, gameId) {
    ...
    const multipleMods = files.filter(file => path.basename(file).toLowerCase() === MOD_FILE.toLowerCase()).length > 1;
    ...
    return Promise.resolve({
        supported: ((gameId === GAME_ID)
            && (files.find(file => path.basename(file).toLowerCase() === MOD_FILE.toLowerCase()))),
        requiredFiles: []
    });
}
